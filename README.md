...
*   **影片時間索引對話**：切換至「對話」分頁即可針對目前影片提問，AI 會依語意將影片切分為時間片段索引，並在回答中引用相關的時間片段，方便快速定位影片內容。
...
8.  **切換至「對話」分頁提問**：
    *   點擊面板上方的「對話」分頁，即可針對目前影片提出問題。
    *   首次提問時，AI 會建立影片的語意時間索引（依主題／事件切分片段），並在回答中引用對應的時間片段作為依據。
    *   後續追問會沿用先前建立的時間索引與對話上下文；點擊「新對話」可清除目前對話並重新開始。
...
*   **Gemini Model**：
    *   指定摘要與對話功能所使用的 Gemini 模型名稱，未填寫時使用擴充功能預設模型。
*   **分析範圍**：
